...
MultiPartUpload.prototype._handleStream = function(stream, callback) {
    ...
    function partReady(part) {
        ...
        if (!part.stream) {
            stream.pause();
            return mpu._uploadPart(part, function (err, result) {
                if (err) { return callback(err); }
                results.push(result);
                delete part.data;
                if (ended) {
...
MultiPartUpload.prototype._uploadPart = function(part, callback) {    
    ...
    // Handle end event and call back
    req.on('end', function() {
        if (result) { return cb(null, result); }
        if (errors.length < 1) { return cb(new Error("recieved 'end' event with no results or errors."));}
        cb(new Error(JSON.stringify(errors[0])));
    });
